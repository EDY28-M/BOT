...
class SuneduBotasaurus:
    ...
    def detectar_estado(self, driver: Driver) -> str:
        ...
        try:
            return driver.run_js("""
                ...
                // Detectar si la página cargó pero no hay nada (ni tabla, ni swal, ni verificacion)
                var input = document.querySelector('input[formcontrolname="dni"]') ||
                            document.querySelector('input[type="text"]');
                if (input) {
                    // La página cargó (hay input), verificar si hay spinner/cargando
                    var spinner = document.querySelector('.p-progress-spinner, .loading, .spinner');
                    if (!spinner) return 'nada';  // Página cargada pero sin resultado alguno
                }
    ...
    def esperar_resultado(self, driver: Driver, timeout: int = 15) -> str:
        ...
        Retorna: 'tabla', 'no_encontrado', 'verificacion', 'verificacion_fallida', 'nada', 'timeout'"""
        inicio = time.time()
        while time.time() - inicio < timeout:
            estado = self.detectar_estado(driver)
            if estado not in ('cargando', 'nada'):
                return estado
            # Si lleva más de 8 segundos en 'nada', considerarlo como timeout
    ...
    def _forzar_refresh(self, driver: Driver):
        ...
        log.info("[F5] Forzando recarga de página, esperando 4 segundos...")
        try:
    ...
    def consultar_dni(self, driver: Driver, dni: str) -> List[Registro]:
        ...
        6. Si verificación fallida / nada / timeout → F5 + 4s y REINTENTAR MISMO DNI
        7. Máximo 5 intentos por DNI
        """
        max_intentos = 5
        intento = 0
        ...
        while intento < max_intentos:
            ...
            try:
                ...
                if self._primera_carga:
                    ...
                    # Reintentos: forzar refresh + 4 segundos
                ...
                # Verificación fallida explícita → F5 + 4s + reintentar
                if estado == 'verificacion_fallida':
                    log.warning("[VERIF] Verificación fallida detectada → F5 + 4s y reintentar")
                ...
                if estado == 'verificacion':
                    ...
                    if estado in ('verificacion', 'verificacion_fallida'):
                        log.warning("[VERIF] No se superó → F5 + 4s y reintentar mismo DNI")
                        self.cerrar_swal(driver)
                        self._forzar_refresh(driver)
                        continue
                    time.sleep(1)
                ...
                if not self.buscar_dni(driver, dni):
                    # Si no pudo buscar → F5 + 4s + reintentar
                ...
                # === VERIFICACIÓN POST-BÚSQUEDA ===
                    ...
                    if post_estado == 'tabla':
                        ...
                        # Verificación no resuelta → F5 + 4s y reintentar
                        log.warning("[VERIF] No superada post-búsqueda → F5 + 4s y reintentar mismo DNI")
                        self.cerrar_swal(driver)
                        self._forzar_refresh(driver)
                        continue

                # === NADA APARECIÓ (página cargada pero sin resultado) ===
                elif resultado == 'nada':
                ...
                log.error(f"[!] Error: {e} → F5 + 4s y reintentar")
                self._forzar_refresh(driver)
